...
【任务要求与流水线执行规范】
1. 审查协同与代码巡检：
   - 检查 docs/code-review/ 下最新的审查报告；如果发现问题优先闭环修复；
   - 深入分析 haier-ac-mac 项目源码与架构，自主寻找 1~2 个高价值优化点，直接完成代码编写与重构。
2. 本地编译与校验：
...
if [ -x "${AGENTAPI_BIN}" ]; then
    ...
    if "${AGENTAPI_BIN}" new-conversation --title="Haier AC 每小时自动巡检优化发版" "${PROMPT_TEXT}" >> "${LOG_FILE}" 2>&1; then
