...
- 🖥 **主窗口控制面板**：
  ...
  - 📈 实时状态胶囊（室内温度/模式/风速）+ 24h 温度趋势曲线（Swift Charts）
- ☁️ **菜单栏迷你面板**：不打开主窗口即可一键开关灯光（`NSStatusItem + NSPopover`，规避 macOS 26 `MenuBarExtra` 幽灵窗口缺陷）
- 🧩 **桌面小组件**：小/中尺寸，实时显示室内温度、目标温度、运行状态（AppGroup 共享快照，状态变化即时刷新）
...
- ⏱ 本地调度：定时/倒计时任务（仅一次/每天/按星期多选），到点自动下发 + 系统通知；情景模式一键应用（可指定设备或全部设备）
- ⚡ 多设备批量控制：批量模式全选/多选设备，同发电源/温度/模式/风速
- 🗣 Shortcuts/快捷指令：开关电源、设置温度、切换模式、应用情景（Siri 可用）
