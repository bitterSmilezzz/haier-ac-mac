...
- 🏷 **模式安全解包与实时音频无锁自愈引擎 (v1.9.24)**：
  ...
  - 🔒 **CoreAudio 实时线程原子无锁快照**：引入 `os_unfair_lock_s` 纳秒级轻量互斥锁与 `@unchecked Sendable` 的 `AudioRenderParameters` 快照器，实时音频渲染回调无锁竞争读取，彻底消灭 TSAN 数据竞争隐患。
