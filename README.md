...
- 🍃 **智能用电能耗估算与节能减排管家 (v1.9.20)**：
  - ⚡ **直流变频实时功率动力学模型**：基于 1.5 匹直流变频压缩机动力学模型，实时根据运行模式（制冷/制热/送风/除湿）、室内外温差梯度 $\Delta T$、风速估算瞬时功率（15W ~ 1650W）
  - 📊 **分钟级能耗积分与 60 天用电账单**：精确积分统计今日耗电量（kWh）与预计电费，保留长达 60 天每日用电记录
  - 💰 **阶梯与峰谷分时电价配置**：支持单一平段电价与峰谷分时电价（默认峰时 08:00~22:00: 0.65¥，谷时 22:00~08:00: 0.35¥）
  - 🍱 **macOS Bento 动态能耗卡片**：7 日用电趋势柱状图、实时瞬时功率胶囊、环保节能评分（Eco Score 40~100）
- 🌧 **自然白噪音与晨间音律助眠联动 (v1.9.20)**：
  - 🎵 **纯算法程序化实时合成**：基于 `AVAudioEngine` + `AVAudioSourceNode` 实时流式运算粉红噪声、布朗噪声与低频相位调制，**零外部音频资源依赖（0 MB 资源包，~0% CPU 极低功耗）**
  - 🌿 **5 大自然沉浸声型**：春夜细雨（细雨滴答）、海风浪涌（低频潮汐）、森林微风（自然气流）、夏夜静谧（夜虫鸣叫）、清晨林鸟（双音轻盈鸣叫）
  - 🌙 **智能睡眠全周期联动**：入眠 2.5s 平滑淡入，进入深睡期（阶段 $\ge 1$）25s 柔和自动淡出，唤醒时刻以「清晨林鸟」12s 淡入播放柔和唤醒
- 🛡 **Wi-Fi 网络与离家/回家感知守护 (v1.9.20)**：
  - 📡 **物理网络与 SSID 状态监听**：基于 `NWPathMonitor` 与 `CoreWLAN` 监听 Mac 网络状态与 Wi-Fi SSID，支持一键将当前网络绑定为家庭网络
  - ⚠️ **离家防空转智能警报**：离家断网超 15 秒且空调仍在运转时，自动发送系统通知（注明设备、状态与设定温），杜绝空转浪费
- 🧼 **空调滤网健康监测与自清洁保养守护 (v1.9.20)**：
  - ⏳ **滤网工时追踪**：开机状态每分钟累计运行时间，基于 250 小时建议清洗周期动态换算洁净度（100% ~ 0%），顶栏常驻状态胶囊（低于 20% 高亮预警）
  - 🧪 **56°C 深度自清洁模式**：向空调下发自清洁指令，提供 20 分钟倒计时动态监控卡片与完成通知
  - 📖 **滤网拆卸水洗 3 步图文指南**：断电扣盖、取网冲洗、阴干复位
- 🌙 **全维智能睡眠温阶体系**：
