...
- 🏷 **闭环自然语言“点五”时间防误降级为倒计时、风速口语与档位全映射、自动模式制热湿度动力学校准及状态栏协同视觉感知 (v1.9.47)**：
  - ⏱️ **自然语言钟点时间“点五”防误降级为倒计时缺陷根治 (`VoiceCommandParser` / `VoiceCommandParserTests`)**：
  ...
  - 🍃 **自然语言风速口语盲区与档位全映射闭环 (`VoiceCommandParser` / `AppModel` / `VoiceCommandParserTests`)**：
  ...
  - ❄️ **自动模式制热分支全气候环境湿度结霜化霜动力学校准 (`EnergyAnalyticsEngine`)**：
  ...
  - 🍱 **macOS 原生状态栏全屋风速协同视觉感知与运行台数精准对齐 (`StatusItemController`)**：
...
- 🏷 **闭环自然语言“X个小时”与两刻时间缺陷、制热全气候湿度动力学、状态栏风速协同矩阵与 Siri 风速快捷指令 (v1.9.46)**：
  - ⏱️ **自然语言“X个小时/半个小时”口语盲区与两刻钟点定时缺陷根治 (`VoiceCommandParser` / `VoiceCommandParserTests`)**：
    - 彻底根除日常高频口语中包含“个”字的小时倒计时失效盲区：重构 `hourPattern` 匹配规则支持 `(?:个?小时|个钟头)`，解决以往“一个小时后关机”、“两个小时后关机”、“2个小时后关机”、“三个小时后关机”直接返回 `nil` 的重大缺陷；并补齐“半个小时后开机/关机”映射；
    - 彻底根治“十点两刻关机”、“十点二刻关机”、“十点2刻关机”因缺少两刻映射被严重误判为 `10:02` 的时间漂移缺陷，精准对齐为 10:30；
    - 补齐“二刻钟”-> 30分钟，以及无“钟”字刻数口语“一刻后”（15分钟）、“两刻后/二刻后”（30分钟）、“三刻后”（45分钟）倒计时解析；
    - 扩充全屋快捷开关机谓词，“全关/全部关/全都关”与“全开/全部开/全都开”无缝映射至全屋电源。
  - ❄️ **制热工况全气候环境湿度热力学动力学校准与滤网机时量纲对齐 (`EnergyAnalyticsEngine` / `AppModel`)**：
    - 在 `EnergyAnalyticsEngine.estimateInstantaneousPower` 的制热工况（`.heating`）中引入环境湿度热力学生态补偿：冬季高湿环境（$\text{RH} \ge 65\%$）模拟室外换热器结霜与高压化霜热负荷补偿（最高 +75W），干燥低湿环境（$\text{RH} \le 40\%$）动态补偿热焓维持功率（最高 +30W），与制冷潜热及自动模式达成 100% 物理对称；
    - 优化 `AppModel.estimatedFilterRemainingDays` 算法：优先采用真实设备机时 `totalDeviceMinutes` 结合设备台数计算日均等效工时，消除以往以墙钟时间估算导致的多设备机时偏差。
  - 🍃 **macOS 原生状态栏全屋与各房间设备风速控制矩阵 (`StatusItemController`)**：
    - 状态栏右键上下文菜单在全屋控制区新增「🍃 全屋风速协同」级联子菜单，直观提供微风、中风、强劲与自动风速统一调度；
    - 在“空调设备控制矩阵”各房间级联子菜单中，新增「🍃 调节风速 (当前: XX)」级联子菜单，支持随时快速调整单机风速档位；单设备模式顶层菜单同步支持快捷风速切换。
  - 🎙️ **Siri / 快捷指令新增风速调节能力 (`AppIntents`)**：
    - 新增 `SetACWindSpeedIntent`，支持通过 Siri 语音或快捷指令自动化调整空调风速（如“用海尔空调设置风速为微风”、“全屋调节风速为强劲”），补全温控四大核心维度的系统级自动化能力。
